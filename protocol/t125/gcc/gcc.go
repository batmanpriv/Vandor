...
import (
	...
	"github.com/batmanpriv/Vandor/protocol/t125/per"
...
func (data *ClientCoreData) Pack() []byte {
	buff := &bytes.Buffer{}
	core.WriteUInt16LE(CS_CORE, buff) 
	core.WriteUInt16LE(0xd8, buff)    
...
func (n *ClientNetworkData) Pack() []byte {
	buff := &bytes.Buffer{}
	core.WriteUInt16LE(CS_NET, buff) 
	length := uint16(n.ChannelCount*12 + 8)
	core.WriteUInt16LE(length, buff) 
...
func (d *ClientSecurityData) Pack() []byte {
	buff := &bytes.Buffer{}
	core.WriteUInt16LE(CS_SECURITY, buff) 
	core.WriteUInt16LE(0x0c, buff)        
...
type ProprietaryServerCertificate struct {
	DwSigAlgId        uint32       `struc:"little"` 
	DwKeyAlgId        uint32       `struc:"little"` 
	PublicKeyBlobType uint16       `struc:"little"` 
	PublicKeyBlobLen  uint16       `struc:"little,sizeof=PublicKeyBlob"`
	PublicKeyBlob     RSAPublicKey `struc:"little"`
	SignatureBlobType uint16       `struc:"little"` 
...
func MakeConferenceCreateRequest(userData []byte) []byte {
	...
	per.WriteChoice(0, buff)                   
	per.WriteSelection(0x08, buff)             
	per.WriteNumericString("1", 1, buff)       
	per.WritePadding(1, buff)                  
	per.WriteNumberOfSet(1, buff)              
	per.WriteChoice(0xc0, buff)                
	per.WriteOctetStream(h221_cs_key, 4, buff) 
