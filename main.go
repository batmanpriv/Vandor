...
func main() {
	...
	if *proto == "ssh" {
		...
		prtl.SetSSHResultCallback(func(host, port, user, pass string) {
			crackedMu.Lock()
			entry := fmt.Sprintf("%s:%s|%s:%s|SSH", host, port, user, pass)
		...
		sshConfig := prtl.SSHCrackerConfig{
			...
			Notify:         *notify,
			SmartPass:      *smartPass,
			PostExploit:    *postExploitFlag,
			ScanNetwork:    *scanNetworkFlag,
			ExtractHash:    *extractHashFlag,
			GenerateScript: *generateScriptFlag,
			ResumeIdx:      startIdx,
			Checkpoint:     cp,
